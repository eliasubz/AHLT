...
# ---------------------------------------------------------------
FEAT_DECODE = {
    'Or': 'ortho', 'EF': 'ext_full', 'EP': 'ext_part',
    'DM': 'drug_morph', 'Gk': 'greek', 'Ln': 'length', 'Sp': 'spacy',
}
...
def parse_filename(fname):
    ...
    meta = {"filename": base, "split": "?", "hidden": "?", "layers": "?",
            "dropout": "?", "emb": "?", "features": "?", "suf_len": "?"}
    ...
    m = re.search(r'_h(\d+)',     rest); meta["hidden"]   = int(m.group(1))   if m else "?"
    m = re.search(r'_l(\d+)',     rest); meta["layers"]   = int(m.group(1))   if m else "?"
    m = re.search(r'_d([0-9.]+)', rest); meta["dropout"]  = float(m.group(1)) if m else "?"
    m = re.search(r'_e([\d-]+)',  rest); meta["emb"]      = m.group(1)        if m else "?"
    m = re.search(r'_f([A-Za-z]+)', rest)
    if m: meta["features"] = decode_feat_str(m.group(1))
    m = re.search(r'_s(\d+)',     rest); meta["suf_len"]  = int(m.group(1))   if m else "?"
...
def main():
    ...
    col_order = ["split", "hidden", "layers", "dropout", "emb", "features", "suf_len",
                 "precision", "recall", "f1",
    ...
    for split, group in df.groupby("split"):
        ...
        def label(row):
            parts = [f"h{row.get('hidden','?')}", f"l{row.get('layers','?')}",
                     f"d{row.get('dropout','?')}"]
        ...
        for i, (metric, color) in enumerate(
            ...
            ax.bar([xi+i*0.25 for xi in x], vals, 0.25,
        ...
        if "hidden" in group.columns and "layers" in group.columns:
            ...
            ax.set_xlabel("Hidden size"); ax.set_ylabel("F1")
